/*
 * NamedTensorTimeSourceMetadata.hpp, part of Hadrons (https://github.com/aportelli/Hadrons)
 *
...
#ifndef Hadrons_NamedTensorTimeSourceMetadata_hpp_
#define Hadrons_NamedTensorTimeSourceMetadata_hpp_
...
/******************************************************************************
 NamedTensorTimeSourceMetadata contains:
...
extern const std::string NamedTensorTimeSourceMetadataFileExtension;

class NamedTensorTimeSourceMetadataDefaultMetadata : Serializable
{
public:
    GRID_SERIALIZABLE_CLASS_MEMBERS(NamedTensorTimeSourceMetadataDefaultMetadata, int, Version);
};

template<typename Scalar_, int NumIndices_, typename MetaData_ = NamedTensorTimeSourceMetadataDefaultMetadata>
class NamedTensorTimeSourceMetadata : Serializable
{
public:
    ...
    public:
    GRID_SERIALIZABLE_CLASS_MEMBERS(NamedTensorTimeSourceMetadata,
    ...
    Eigen::Index NamedTensorTimeSourceMetadataSize() { return 1; }
    template<typename... IndexTypes>
    inline Eigen::Index NamedTensorTimeSourceMetadataSize(Eigen::Index firstDimension, IndexTypes... otherDimensions)
    {
        if( sizeof...(otherDimensions) )
            firstDimension *= NamedTensorTimeSourceMetadataSize( otherDimensions... );
        return firstDimension;
    }

    // Construct a named tensor explicitly specifying size of each dimension
    template<typename... IndexTypes>
    NamedTensorTimeSourceMetadata(const std::string &Name,
                                                      const std::array<std::string, NumIndices_> &indexNames,
                                                      Eigen::Index firstDimension, IndexTypes... otherDimensions)
    : deviceBuf( Traits::count * NamedTensorTimeSourceMetadataSize(firstDimension, otherDimensions...) ),
	...
	{
            HADRONS_ERROR(Argument, "NamedTensorTimeSourceMetadata: dimensions != tensor rank");
	}
    }
    ...
    void write(const std::string &FileName, const std::string &Tag) const
    {
        ...
        FileName_.append( NamedTensorTimeSourceMetadataFileExtension );
    ...
    template<typename Reader> void read(Reader &r, bool bValidate, const std::string &Tag)
    {
        ...
        for (int i = 0; i < NumIndices_; i++)
            ...
            {
                HADRONS_ERROR(Size,"NamedTensorTimeSourceMetadata::read dimension size");
            }
        if (bValidate && !ValidateIndexNames(OldIndexNames))
        {
            HADRONS_ERROR(Definition,"NamedTensorTimeSourceMetadata::read dimension name");
        }
    }
    ...
    {
        Default_Reader r(FileName + NamedTensorTimeSourceMetadataFileExtension);
...
class PerambMetadataTimeSourceMetadata : Serializable
{
public:
    GRID_SERIALIZABLE_CLASS_MEMBERS(PerambMetadataTimeSourceMetadata, int, Version,
                                    std::vector<std::vector<int>>, sourceTimes );
};

class PerambTensorTimeSourceMetadata : public NamedTensorTimeSourceMetadata<SpinVector, 6, PerambMetadataTimeSourceMetadata>
{
    public:
    ...
    // Construct a named tensor explicitly specifying size of each dimension
    template<typename... IndexTypes>
    PerambTensorTimeSourceMetadata(Eigen::Index nT, Eigen::Index nVec, Eigen::Index LI, Eigen::Index nNoise, Eigen::Index nT_inv, Eigen::Index SI)
    : NamedTensorTimeSourceMetadata{Name__, DefaultIndexNames__, nT, nVec, LI, nNoise, nT_inv, SI} {}
};


END_MODULE_NAMESPACE
END_HADRONS_NAMESPACE
#endif // Hadrons_NamedTensorTimeSourceMetadata_hpp_
